...
import com.example.scciapp.AC.ACWorkshopFragment;
...
public class HeadACHomeActivity extends AppCompatActivity {
	private Boolean isPaused = false;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_head_achome);
...
//						.commit();
					...
					getSupportFragmentManager()
						.beginTransaction()
						.setReorderingAllowed(true)
						.replace(R.id.fragmentContainerView, HeadAnnouncementsFragment.class, null)
						.commit();
				} else if(item.getItemId() == R.id.workshop){
					getSupportFragmentManager()
						.beginTransaction()
						.setReorderingAllowed(true)
						.replace(R.id.fragmentContainerView, HeadACWorkshopFragment.class, null)
						.commit();
				}
			...
			HttpClient.getClient().newCall(request).enqueue(new Callback() {
				...
				public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
					...
					if (response.isSuccessful() && response.code() == 200 && responseBody != null) {
						...
						Intent intent = new Intent(HeadACHomeActivity.this, AuthActivity.class);
	...
	protected void onResume() {
		super.onResume();
		if (isPaused){
			HeadACHomeActivity.this.recreate();
			isPaused = false;
		}
	}
}