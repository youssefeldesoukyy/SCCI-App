...
public class ParticipantHomeActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getSupportActionBar().setTitle("Participant " + Session.currentUser.getUserWorkshop());
			...
			public boolean onNavigationItemSelected(@NonNull MenuItem item) {
				if (item.getItemId() == R.id.profile) {
					...
					getSupportFragmentManager()
						.beginTransaction()
						.setReorderingAllowed(true)
						.replace(R.id.fragmentContainerView, ParticipantWorkshopFragment.class, null)
						.commit();
				}
			...
			HttpClient.getClient().newCall(request).enqueue(new Callback() {
				...
				public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
					...
					if (response.isSuccessful() && response.code() == 200 && responseBody != null) {
						...
						Intent intent = new Intent(ParticipantHomeActivity.this, AuthActivity.class);
	...
	protected void onResume() {
		super.onResume();
		if (isPaused){
			ParticipantHomeActivity.this.recreate();
			isPaused = false;
		}
	}
}