...
public class HeadHomeActivity extends AppCompatActivity {
			...
			HttpClient.getClient().newCall(request).enqueue(new Callback() {
				...
				public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
					...
					if (response.isSuccessful() && response.code() == 200 && responseBody != null) {
						...
						Intent intent = new Intent(HeadHomeActivity.this, AuthActivity.class);
	...
	protected void onResume() {
		super.onResume();
		if (isPaused){
			HeadHomeActivity.this.recreate();
			isPaused = false;
		}
	}
}